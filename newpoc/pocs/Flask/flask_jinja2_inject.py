#!/usr/bin/env python3
# -*- coding:utf-8 -*-
from newpoc.api import requests, urljoin, PocBase, Output
...
class POC(PocBase):
    ...
    def _verify(self, url):
        ...
        # flag = 1048576
        # result = {}
        # url = url.format("{{1024*1024}}")
        # recv = requests.get(url)
        # print(recv.text)
        # print('正在测试flask:{}'.format(url))
        # if str(flag) in recv.text:
        #     # result['VerifyInfo'] = {}
        #     # result['VerifyInfo']['URL'] = 'zheliok'
        #     # result['VerifyInfo']['Postdata'] = recv.text
        #     print('ok')
        # else:
        #     print('no')
        # print('测试flask完毕')
        ...
        # # return output
        self.output.check("flask_verify:{}".format(url))

    def _attack(self, url):
        self.output.check("flask_attack:{}".format(url))
