...
class POC(PocBase):
    ...
    pocDesc = "pocs/Redis/redisnone.py"
    ...
    def _verify(self, ip):
        ...
        try:
            ...
            if 'redis_version' in recvdata:
                self.output.success('redisdone:{} is vulnerable '.format(ip))
                result['target-url'] = result_url
                POC_QUEUE.put(result)
            else:
                self.output.fail('redisdone:{} is not vulnerable '.format(ip))
        except Exception as err:
            self.output.fail('redisdone:{0} is fail {1}'.format(ip, err))
